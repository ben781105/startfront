...
function About(){
  ...
  return(
    <section id='about' className=' bg-[#ffffff] dark:bg-[#050816] h-fit flex flex-col w-full max-w-screen md:px-10 lg:px-30  pl-4 pr-4 pt-20 pb-10 justify-center overflow-x-hidden'>
       ...
       <div className='w-full px-6 pt-3 pb-6 border-b-2 bg-[#f3f4f6]  border-[#915eff] dark:bg-[#1d1836] rounded-lg'>
      
       
<span className="flex flex-col gap-3 w-full mt-4">
    <h1 className='text-[#1a1a1a] dark:text-[#ffffff] mb-4 font-semibold  text-2xl 2xl:text-3xl '>Expertise</h1>
<ul className="flex flex-col gap-3 w-full ">
  {content.map((c) => (
    <li
      key={c.id}
      className="relative  pl-5 xs:text-sm sm:text-base 2xl:text-xl text-[#1a1a1a] dark:text-white  before:content-[''] before:absolute before:left-0 before:top-1/3 before:-translate-y-1/2 before:w-2 before:h-2 before:rounded-full dark:before:bg-[#ffffff] before:bg-[#1a1a1a]"
    >
      {c.service}
    </li>
  ))}
       ...
       <div className='w-full border border-[#915eff] dark:border-[#915eff] h-90 dark:bg-[#090325] flex items-center justify-center rounded-lg'>
