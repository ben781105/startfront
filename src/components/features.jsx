...
function Features(){
  ...
  return(
    <section className="h-fit pr-4 pl-4 pt-8 overflow-x-hidden md:pl-20 md:pr-20 space-y-2 flex flex-col mt-5">
      ...
      <p className='self-center text-center mt-6'>Bulk SMS sending, SMS Campaign Creation, Delivery Reports and Anaytics, Api Integration</p>
      <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-4 overflow-x-hidden'>
        {features.map((feature)=>
         <div className='flex w-full lg:p-4 md:pl-6 md:pr-6 justify-center items-center flex-col p-3 text-center bg-gray-300 shadow-md rounded-lg ' 
         key={feature.id}
         >
            <img src={feature.icon} alt="featute-icon" width={40}/>
            <h2 className='text-2xl text-semibold mt-3 '>{feature.name}</h2>
            <p className='text-base mt-2'>{feature.desc}</p>
         </div>
        )}
      </div>
    </section>
